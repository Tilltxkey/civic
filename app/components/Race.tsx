...
function BarLegend({ slots }: { slots: { name: string; votes: number; pct: number; color: string }[] }) {
  const C = useC();
  return (
...
export default function Race({ selectedRaceId }: RaceProps) {
  ...
  // Build slots — always pad to minimum 2 so slots[0] and slots[1] are never undefined
...
function Countdown({ selectedRaceId }: { selectedRaceId: string }) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      setSecs(s);
      // When the timer hits zero while election is still "ongoing",
      // re-fetch so every client picks up the DB status change promptly.
      if (s === 0 && election?.status === "ongoing") {
        refreshElection();
      }
    };
    update();
